...
from src.config_browser import ConfigBrowser
...
"""
    ...
    def apply_sample_config(self, config_entry, serial_port: str = "") -> bool:
        """Apply a sample config from the config browser."""
        from src.config_browser import apply_config

        source_path = config_entry.source_path.parent.parent
        success = apply_config(
            source_path=source_path,
            filename=config_entry.filename,
            dest_path=self.printer_cfg,
            serial_port=serial_port or self.config.serial_port,
        )
        if success:
            logger.info(f"Applied sample config: {config_entry.filename}")
        return success
...
class InstanceManager:
    ...
    def __init__(self, config_mgr: ConfigManager):
        ...
        self._state_callbacks: List[Callable] = []
        self.config_browser = ConfigBrowser(KLIPPER_PATH, KALICO_PATH)
